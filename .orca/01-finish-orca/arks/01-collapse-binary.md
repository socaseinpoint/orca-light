...
- archiving an ark appends a line to its focus's `## Журнал`; archiving a focus moves its dir to `.orca/archive/`.
- the legacy `.orca/arks/verify-resume.md` and global `.orca/decisions.md` are migrated into `01-finish-orca/` (verify-resume → an archived ark, since its deliverable is done).
...
- map the current `bin/orca` commands (`now`, `verify`, `decide`, `ark-root`, `trail`, `archive`, `gate`) onto the focus-dir model; decide which survive, which are cut (`ark-root` cut), which change shape (`now` reads local; `archive` moves folders).
